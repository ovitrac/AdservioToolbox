...
#### Default Policy (`mcp_policy.yaml`) — 10 rules

| ID | Type | Severity | What it detects |
|----|------|----------|----------------|
| `ssh_private_key` | regex | critical | SSH/RSA/DSA/EC/ED25519 private keys |
| `pem_certificate` | regex | critical | X.509 PEM certificates |
| `aws_access_key` | regex | critical | AWS access keys (AKIA/ASIA + 16 chars) |
| `gcp_api_key` | regex | critical | GCP API keys (AIza...) |
| `jwt_token` | regex | critical | JWT tokens (3-part base64) |
| `email` | regex | medium | Email addresses (with whitelist) |
| `ipv4` | ipv4 | medium | IPv4 addresses (with CIDR whitelist) |
| `ipv6` | ipv6 | medium | IPv6 addresses |
| `url` | url | medium | HTTP/HTTPS URLs |
| `high_entropy_token` | entropy | medium | High-entropy strings (≥4.5 bits, ≥20 chars) |

#### Enterprise Policy (`mcp_policy_enterprise.yaml`) — 26 rules

Inherits all 10 default rules, adds 16 provider-specific rules:

| ID | Type | Severity | What it detects |
|----|------|----------|----------------|
| `github_pat` | regex | critical | GitHub Personal Access Tokens (ghp_...) |
| `github_fine_grained` | regex | critical | GitHub fine-grained tokens (github_pat_...) |
| `gitlab_token` | regex | critical | GitLab tokens (glpat-...) |
| `slack_bot_token` | regex | critical | Slack bot tokens (xoxb-...) |
| `slack_webhook` | regex | critical | Slack webhook URLs (hooks.slack.com) |
| `stripe_secret_key` | regex | critical | Stripe live secret keys (sk_live_...) |
| `stripe_test_key` | regex | medium | Stripe test keys (sk_test_...) |
| `npm_token` | regex | critical | npm tokens (npm_...) |
| `heroku_api_key` | regex | critical | Heroku API keys (UUID format) |
| `twilio_api_key` | regex | critical | Twilio API keys (SK...) |
| `sendgrid_api_key` | regex | critical | SendGrid API keys (SG...) |
| `generic_password_assign` | regex | medium | Password assignments in config files |
| `generic_secret_assign` | regex | medium | Secret/token assignments in config files |
| `private_key_pkcs8` | regex | critical | PKCS#8 private keys |
| `encrypted_private_key` | regex | critical | Encrypted private keys |
| `azure_connection_string` | regex | critical | Azure connection strings (AccountKey=...) |

The enterprise policy also overrides the base `high_entropy_token` rule with whitelist patterns for data URIs.

### Bash Safety Guard — Blocked Patterns (hardened only)

The `cloak-safety-guard.sh` hook (PreToolUse for Bash) blocks 13 command patterns:

| # | Pattern | Description |
|---|---------|-------------|
| 1 | `rm -rf /` | Recursive delete root |
| 2 | `sudo rm` | Privileged delete |
| 3 | `curl \| sh` | Remote code execution (curl) |
| 4 | `curl \| bash` | Remote code execution (curl) |
| 5 | `wget \| sh` | Remote code execution (wget) |
| 6 | `wget \| bash` | Remote code execution (wget) |
| 7 | `git push --force` | Force push |
| 8 | `git push -f` | Force push (short flag) |
| 9 | `git reset --hard` | Discard all changes |
| 10 | `git clean -f` | Remove untracked files |
| 11 | `chmod -R 777` | World-writable permissions |
| 12 | `dd ... of=/dev/` | Raw disk write |
| 13 | `mkfs` | Format filesystem |

All patterns are regex-based (defined in `cloakmcp/hooks.py`). Matching is case-sensitive.
...
| `cloak-guard-read.sh` | PreToolUse | `Read\|Grep\|Glob` | Block reads outside project tree (hardened only) |
