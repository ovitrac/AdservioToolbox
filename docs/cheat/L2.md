...
```
Level 0: memctl push --source  ← ingest first (cold start — memory empty)
...
Rule: do not skip levels. If memory is empty, start at Level 0. Narrow the query before falling back to native Read.
